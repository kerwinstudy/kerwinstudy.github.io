	...
	$.ajax({
		...
		success:function(data){
			if(data.delResult == "true"){//删除成功：移除删除行
				...
				changeDLGContent("Sorry，failed to delete user【"+obj.attr("username")+"】");
			}else if(data.delResult == "notexist"){
				//alert("对不起，用户【"+obj.attr("username")+"】不存在");
				changeDLGContent("Sorry，user【"+obj.attr("username")+"】 is not exist");
			}
		},
		error:function(data){
			//alert("对不起，删除失败");
			changeDLGContent("Sorry,failed to delete");
		}
	});
}
